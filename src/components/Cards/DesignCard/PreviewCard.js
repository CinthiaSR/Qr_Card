...
const PreviewCard = ({ mostrarQR, onClick }) => {
    return (
        <div className="bg-white p-6 shadow-md h-full">
            <h3 className="text-center font-semibold mb-4">Vista Previa</h3>
            <div className="flex justify-center mb-4">
                {mostrarQR ? (
                    <QrCodeIcon className="h-46 w-46 text-[#e79363]" />
                ) : (
                    <UserCircleIcon className="h-46 w-46 text-[#e79363]" />
                )}
            </div>
            ...
            <div className="flex justify-center mt-4">
                <button onClick={onClick} className="bg-[#009688] text-white px-6 py-2 rounded-lg transition">
